...
spawn_same_session_send(void *arg) 
{
  ...
  // Just use SSL_write/SSL_read to do the handshake
  int ret = SSL_write(ssl, req_buf, strlen(req_buf));
  int read_count = 0;
  int write_count = 1;
  char input_buf[1024];
  ...
  while (ret < 0) {
    ...
    } else if (FD_ISSET(sfd, &writes)) {
      ret = SSL_write(ssl, req_buf, strlen(req_buf));
    }
  } 

  // Should be done writing by now
...
//  printf("total_bytes=%d handshake writes=%d handshake reads=%d\n", total_read, write_count, read_count);
